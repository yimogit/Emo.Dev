<template>
  <div class="my-layout">
          ...
          <el-form :inline="true" label-width="auto" @submit.stop.prevent>
            <el-form-item class="my-search-box-item" label="所属项目">
              <el-select clearable v-model="state.filter.projectId" placeholder="" @keyup.enter="onQuery">
                <el-option v-for="item in state.selectDevProjectListData" :key="item.id" :value="item.id"
                  :label="item.name" />
              </el-select>
            </el-form-item>
            <el-form-item>
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item v-if="auth(perms.batSoftDelete)" :disabled="state.sels.length == 0"
                    @click="onBatchSoftDelete" icon="ele-DeleteFilled">批量软删除</el-dropdown-item>
    ...
    <el-card class="my-fill mt8" shadow="never">
      <el-table v-loading="state.loading" :data="state.devProjectGenListData" row-key="id" ref="listTableRef"
        ...
        <el-table-column v-auths="[perms.update, perms.softDelete, perms.delete]" label="操作" fixed="right">
          <template #default="{ row }">
            <el-button v-auth="perms.update" icon="ele-EditPen" text type="primary"
              @click.stop="onEdit(row)">编辑</el-button>
            <el-button v-auth="perms.update" icon="ele-View" text type="primary"
              @click.stop="onPreview(row)">预览</el-button>
            <el-button v-auth="perms.update" icon="ele-Position" text type="primary"
              @click.stop="genCode(row)">生成</el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <div class="my-flex my-flex-end" style="margin-top: 20px">
        <el-pagination v-model:currentPage="state.pageInput.currentPage" v-model:page-size="state.pageInput.pageSize"
          :total="state.total" :page-sizes="[10, 20, 50, 100]" background @size-change="onSizeChange"
...
import {
  PageInputDevProjectGenGetPageInput, DevProjectGenGetPageInput, DevProjectGenGetPageOutput, DevProjectGenGetOutput, DevProjectGenAddInput, DevProjectGenUpdateInput,
  DevProjectGenGetListInput, DevProjectGenGetListOutput,
...
const onBatchSoftDelete = async () => {
  proxy.$modal?.confirmDelete(`确定要将选择的${state.sels.length}条记录移入回收站？`).then(async () => {
    const rst = await new DevProjectGenApi().batchSoftDelete(state.sels?.map(item => item.id) as number[], { loading: true, showSuccessMessage: true })
    if (rst?.success) {
      onQuery()
    }
  })
