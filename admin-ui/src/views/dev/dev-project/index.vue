      ...
      <el-row>
        <el-col :span="18" :xs="24" class="my-search-box-inputs">
          <el-form :inline="true" label-width="auto" @submit.stop.prevent>
            ...
            <el-form-item class="my-search-box-item" label="项目编码">
        ...
        <el-col :span="6" :xs="24" class="my-search-box-btns">
          <el-space>
            ...
            <el-dropdown :placement="'bottom-end'" v-if="auths([perms.batSoftDelete, perms.batDelete])">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item :disabled="state.sels.length == 0" @click.stop="batchGenCode"
                    icon="ele-Position">批量生成代码</el-dropdown-item>
        ...
        <el-table-column v-auths="[perms.update, perms.softDelete, perms.delete]" label="操作" :width="actionColWidth"
          ...
          <template #default="{ row }">
            ...
            <el-dropdown v-if="authAll([perms.delete, perms.softDelete])">
              <el-button icon="el-icon--right" text type="warning">操作 <el-icon class="el-icon--right">
                  <component :is="'ele-ArrowDown'" />
                </el-icon></el-button>
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item @click.stop="genCode(row)" icon="ele-Position">生成代码</el-dropdown-item>
                  <el-dropdown-item v-if="auth(perms.softDelete)" @click.stop="onSoftDelete(row)"
                    icon="ele-DeleteFilled">软删除</el-dropdown-item>
                  <el-dropdown-item v-if="auth(perms.delete)" @click.stop="onDelete(row)"
                    icon="ele-Delete">删除</el-dropdown-item>
                </el-dropdown-menu>
              </template>
            </el-dropdown>
            <span v-else style="margin-left:5px;height:inherit">
              <el-button text type="warning" v-if="auth(perms.softDelete)" style="height:inherit"
                @click.stop="onDelete(row)" icon="ele-DeleteFilled">软删除</el-button>
              <el-button text type="danger" v-if="auth(perms.delete)" style="height:inherit" @click.stop="onDelete(row)"
